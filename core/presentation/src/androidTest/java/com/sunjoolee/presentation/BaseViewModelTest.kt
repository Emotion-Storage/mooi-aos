...
data class TestState(val dummy: Int = 0)
class TestViewModel : BaseViewModel<TestState>(TestState()) {
    fun throwInBaseIntent(error: Throwable) = baseIntent {
        throw error
    }

    fun throwInScope(error: Throwable) = baseViewModelScope.launch {
        throw error
    }
}
...
class BaseViewModelTest {
    ...
    @Test
    fun baseIntent_should_catch_network_error() = runTest {
    ...
    @Test
    fun baseIntent_should_catch_auth_error() = runTest {
        val error = BaseException(
            message = "expired",
    ...
    @Test
    fun baseIntent_should_catch_internal_server_error() = runTest {
        val error = BaseException(
            message = "server down",
    ...
    @Test
    fun baseIntent_should_convert_unknown_error() = runTest {
    ...
    @Test
    fun baseViewModelScope_should_catch_errors() = runTest {
        val error = BaseException(
            message = "server fail",
            code = ErrorCode.INTERNAL_SERVER_ERROR
        )
    ...
    @Test
    fun baseViewModelScope_should_convert_unknown_error() = runTest {
